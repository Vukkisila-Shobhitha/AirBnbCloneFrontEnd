...
function Header() {
  ...
  return (
    <div className={`App ${hasShadow ? 'has-shadow' : ''}`}>
      <header>
        <div className="d-flex header-comp">
          <Link to="/" className="App text-decor">
            <svg xmlns="http://www.w3.org/2000/svg" className="logo" viewBox="0 0 448 512" style={{ fill: "rgb(255,56,92)" }}>
            ...
            <span className="app-name" style={{ color: "rgb(255,56,92)" }}>airbnb</span>
          </Link>

          {showSearchBar && <SearchBar />}

          <Link to={user ? "/userprofile" : "/signin"} className="header-nav App shadow text-decor">
            <FontAwesomeIcon className="profile-logo-bar header-comp3" icon={faBars} />
            <FontAwesomeIcon className="profile-logo header-comp3" icon={faCircleUser} aria-label="User Profile" />
